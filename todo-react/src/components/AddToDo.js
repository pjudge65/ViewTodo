...
export default function AddToDo({todos, setTodos, loadTodos, date, undoRedoActions, setUndoRedo, createTodo, projects}) {


    // sets the default input values for the add-todo form
    const [inputs, setInputs] = useState({title: "", dueDate: "", project: "Inbox", priority: "1", notes: ""});
    ...
    const handleAddTodo = async (event) => {
        ...
        setInputs({title: "", dueDate: "", project: "Inbox", priority: "1", notes: ""});
    ...
    return (
        <div id='add-todo-button'>
            ...
            <Popup
                trigger={
                   <button type="button" className="button" id="add-td-btn" onClick={()=>setOpen(open=> !open)}>+</button>
                }
                position={'top right'}
                on={['hover']}
                arrow={true}
            ...
            >Create New To-do</Popup>

            {/* The add-todo form itself */}
            <Popup open={isOpen} modal nested onClose={closePopup} className="todoFormPopup">
