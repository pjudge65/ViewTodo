...
export default function ReturnImport({
  ...
  const integrateCalendarData = async function () {
    ...
    for (let i = 0; i < importResults.length; i++) {
      ...
      let notesString = `[${processTimeNotes(importResults[i])}] ${
        importResults[i].notes
      }`;
  ...
  const handleImport = async function (event) {
    ...
    const response = await fetch(
      `http://localhost:8000/api/appointments?date=${dateString}T00:00`
    );
      ...
      <div id="return-import-div">
        ...
        <div id="import-button-div">
          ...
          >
            <div className="modal">
              <h4>
                Import Calendar Appointments (from Microservice) to
                automatically generate to-do items?
              </h4>
              <h5>Warning: Importing might affect current to-dos.</h5>
              <button id="import-yes" onClick={handleImport}>
                Yes
              </button>
              <button id="import-no" onClick={closePopupImport}>
                No
              </button>
            </div>
          </Popup>
          ...
          <Popup
            open={isOpenImportResult}
            closeOnDocumentClick
            onClose={closePopupImportResult}
          >
            <div className="modal">
              <ImportResultForm
